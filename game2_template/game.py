...
def print_menu(exits, room_items, inv_items):
    ...
    for direction in exits:
        ...
        print("GO " + direction.upper() + " to " + destination + ".")
...
def check_victory():
    """Checks if all items are in the Reception room.
    Returns True if the player has won, otherwise False."""

    reception = rooms["Reception"]
    #check all rooms have no items 

    if len(rooms["Admins"]["items"]) > 0:
        return False
    if len(rooms["Tutor"]["items"]) > 0:
        return False
    if len(rooms["Parking"]["items"]) > 0:
        return False
    if len(rooms["Office"]["items"]) > 0:
        return False

    if len(inventory) > 0: #player has no items in inv
        return False
...
def main():
    ...
    while True:
        ...
        if check_victory():
            print()
            print("Congratulations! You have collected all the items and returned them to the Reception!")
